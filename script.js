...
/* Manual overrides:
   If a card still fails to find its image, paste the exact filename from your
   certificates folder here (value must include the 'certificates/' prefix).
   Example:
   "1_Foundations_of_Digital_Marketing_and_E-commerce": "certificates/1_Foundations_of_Digital Marketing_and_E-commerce.jpg"
*/
const OVERRIDES = {
  // add entries only if something fails to load
};
...
const LIST = [
  ...
  {group:"course", order:17, title:"Fundamentals of Building AI Agents", key:"23_Fundamentals_of_BuildING_AI_Agents".toLowerCase().replace("ing","ing"), url:"https://coursera.org/verify/FT2JAV8QC5I9"},
...
function buildSrcVariants(key){
  ...
  // keep underscores, drop commas
  variants.add(reps(base));
  // spaces + keep ampersands
  variants.add(base.replaceAll("_"," ").replaceAll("__","_"));
  // e-commerce spelling variants
  variants.add(base.replaceAll("E-commerce","E commerce"));
  variants.add(base.replaceAll("E commerce","E-commerce"));
  // collapse multiple spaces (encoded by browser automatically)
  variants.add(reps(base.replaceAll("_","  "))); // double spaces
